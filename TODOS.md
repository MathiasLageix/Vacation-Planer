...
### Race condition dans `get_or_create_session` (TOCTOU)
**Priority:** P2
Deux requêtes simultanées avec des critères identiques peuvent créer deux sessions avec des UUIDs différents (confirmé par Red Team review). Les snapshots s'accumulent sur des sessions distinctes, les insights ne détectent plus les changements de prix. Fix : `UniqueConstraint("search_type", "criteria_hash")` + `INSERT OR IGNORE` via `on_conflict_do_nothing`.
...
**Completed:** v0.5.0.0 (2026-06-17) — None-sentinel pattern, singleton FastAPI lifespan.
...
### Refactoring : search_core → architecture brittle
**Priority:** P2
`search_core()` retourne un `dict` que `run_search()` re-hydrate manuellement en dataclasses pour le display — tout nouveau champ sur `NormalizedFlight` ou `FlightSegment` doit être mis à jour en 2 endroits. Fix : extraire `search_core_objects()` retournant les dataclasses brutes, et séparer la sérialisation dans `api/serializers.py`. Aussi : déplacer `search_core()` de `main.py` (CLI) vers un module `core.py` pour éviter les side effects de `load_dotenv()` à l'import.

### Datetime timezone naive dans SQLite
**Priority:** P3
`captured_at` est inséré comme UTC mais lu par SQLAlchemy comme datetime naïf (sans tzinfo). Les timestamps dans les insights sont ambigus pour les utilisateurs en dehors d'UTC. Fix : `DateTime(timezone=True)` sur la colonne ou `.replace(tzinfo=timezone.utc)` après lecture.

### Tests : couverture erreurs fournisseurs
**Priority:** P2
Plusieurs chemins d'erreur ne sont pas testés : (1) hotel provider lève une exception → `hotels=[]` silencieux, sans `hotel_error` dans le résultat ; (2) `flexible_days` partiel où certaines dates échouent mais d'autres réussissent ; (3) test intégration `GET /api/sessions` après un vrai `POST /api/search` (sans mock Storage).
